...
<template>
  <div class="cov-vue-date">
    <div class="datepickbox">
      <input type="text" class="cov-datepicker" readonly v-model="date.time" :required="required" @click="showCheck" @foucus="showCheck" :style="option.inputStyle ? option.inputStyle : {}" />
    </div>
      ...
      <div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
        ...
        <div class="cov-date-box" v-if="showInfo.day">
          <div class="cov-picker-box">
            ...
            <div class="day" v-for="day in dayList" track-by="$index" @click="checkDay(day)" :class="{'checked':day.checked,'unavailable':day.unavailable,'passive-day': !(day.inMonth)}" :style="day.checked ? (option.color && option.color.checkedDay ? { background: option.color.checkedDay } : { background: '#2fbab7' }) : {}">{{day.value}}</div>
          </div>
        </div>
        ...
        <div class="button-box">
          <span class="cancel" @click="showInfo.check=false">{{option.buttons? option.buttons.cancel : 'Cancel' }}</span>
          <span class="ok" @click="picked">{{option.buttons? option.buttons.ok : 'Ok'}}</span>
        </div>
      </div>
    </div>
  </div>
