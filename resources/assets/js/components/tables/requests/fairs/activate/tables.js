...
var _tableUpdate   = function (instance, request) {
    $('#date-search-request').on('click', function() {
        ...
        if (window.moment(todayDate) >= window.moment(dateFrom) &&
            ...
            // Request Data
            tablesRequest(request, true, instance, _payloadMount(dateFrom, dateTo, brand));
        }
    });
};
 ...
 * @private
 */
var tablesRequest = function (_request, isUpdate = false, instance = null, payload = null) {
...
var _table = function() {
    ...
    for (var i = 0, len = _request.length; i < len; i++) {
        window.closedTable = new Vue({
            el: '#' + _request[i],
            mixins: [Tables],

            beforeCreate() {
                // After mounted request data
                tablesRequest(_request[i], false, this, _payloadMount());
